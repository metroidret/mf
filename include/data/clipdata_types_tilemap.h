#ifndef CLIPDATA_TYPES_TILEMAP_H
#define CLIPDATA_TYPES_TILEMAP_H

#include "types.h"
#include "constants/clipdata.h"

extern const u8 sClipdataCollisionTypes[CLIPDATA_COUNT];

extern const u8 sClipdataCollisionTypes_Test[CLIPDATA_TEST_COUNT];

extern const u8 sTileset_0_Tilemap[1604];

extern const u16 sTileset_0_Pal[14 * 16];

extern const u16 sClipdataBehaviorTypes[CLIPDATA_COUNT];

extern const u16 sClipdataBehaviorTypes_Test[CLIPDATA_TEST_COUNT];

#endif /* CLIPDATA_TYPES_TILEMAP_H */
