...
void Dessgeega(void)
{
    ...
    if (gCurrentSprite.pose < SPRITE_POSE_TURNING_INTO_X)
    {
        if (SPRITE_GET_ISFT(gCurrentSprite) == 4)
        {
            ...
            if (gCurrentSprite.status & SPRITE_STATUS_ONSCREEN &&
                SpriteUtilFindSecondaryWithRamSlot(SSPRITE_DESSGEEGA_SPIKE, gCurrentSprite.primarySpriteRamSlot) <= 2)
            {
                if (gCurrentSprite.status & SPRITE_STATUS_Y_FLIP)
                {
                    SpriteSpawnSecondary(SSPRITE_DESSGEEGA_SPIKE, gCurrentSprite.roomSlot,
                        gCurrentSprite.spritesetGfxSlot, gCurrentSprite.primarySpriteRamSlot,
                        gCurrentSprite.yPosition + BLOCK_TO_SUB_PIXEL(1.1875f), gCurrentSprite.xPosition - BLOCK_SIZE,
                        0);
                    SpriteSpawnSecondary(SSPRITE_DESSGEEGA_SPIKE, gCurrentSprite.roomSlot,
                        gCurrentSprite.spritesetGfxSlot, gCurrentSprite.primarySpriteRamSlot,
                        gCurrentSprite.yPosition + BLOCK_TO_SUB_PIXEL(1.1875f), gCurrentSprite.xPosition + BLOCK_SIZE,
