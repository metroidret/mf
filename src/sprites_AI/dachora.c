...
void DachoraWaitingForOthers(void) {
  ...
  for (count = 0, i = 0; i < 0x18; i++) {
    if (gSpriteData[i].status & 1 && !(gSpriteData[i].properties & 0x80) &&
        gSpriteData[i].spriteId == 0xB6 && gSpriteData[i].pose == 0x1C)
      count++;
  }
...
void DachoraWaitingToSpawnBaby(void) {
  ...
  if (gCurrentSprite.work1 == 0) {
    ...
    SpriteSpawnSecondary(0x69, 0, gCurrentSprite.spritesetGfxSlot,
                         ...
                         gCurrentSprite.xPosition - 0xC0, 0x40);
...
void BabyDachoraLeavingEnclosure(void) {
  ...
  if (sArray_0x0838188c[var] == 0x7FFF) {
    gCurrentSprite.pose = 0x17;
    gCurrentSprite.work1 = 0x28;
    gCurrentSprite.bgPriority = 2;
  } else {
    gCurrentSprite.work4 = var + 1;
    gCurrentSprite.yPosition += sArray_0x0838188c[var];
    gCurrentSprite.xPosition += 4;
  }
}
...
void BabyDachoraWaitingToRun(void) {
  ...
  while (i < 0x18) {
    if (sprites[i].status & 1 && !(sprites[i].properties & 0x80) &&
