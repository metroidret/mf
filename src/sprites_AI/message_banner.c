...
#define MESSAGE_BANNER_YES_X 54
#define MESSAGE_BANNER_NO_X 150
...
u8 TrySpawnMessageBanner(u8 type)
{
    ...
    {
        if (gSpritesetSpriteIds[i] == PSPRITE_MESSAGE_BANNER)
        {
    ...
    if (gfxRow < 8)
    {
        i = SpriteSpawnPrimary(PSPRITE_MESSAGE_BANNER, type, gfxRow, SSP_UNINFECTED_OR_BOSS,
...
void MessageBannerInit(void)
{
    ...
    gCurrentSprite.pOam = sMessageBannerOam_PopUp;
...
void MessageBannerGfxInit(void)
{
    ...
    if (gCurrentSprite.workY < sizeof(sMessageBannerGfx) / 512)
        SpriteLoadGfx(PSPRITE_MESSAGE_BANNER, gCurrentSprite.spritesetGfxSlot, gCurrentSprite.workY);
    else if (gCurrentSprite.workY == sizeof(sMessageBannerGfx) / 512)
        SpriteLoadPal(PSPRITE_MESSAGE_BANNER, gCurrentSprite.spritesetGfxSlot, ARRAY_SIZE(sMessageBannerPal) / 16);
...
void MessageBannerPopUp(void)
{
    ...
    {
        gCurrentSprite.pOam = sMessageBannerOam_Static;
        gCurrentSprite.animationDurationCounter = 0;
        gCurrentSprite.currentAnimationFrame = 0;
        ...
        if (roomSlot == 0)
        {
            gCurrentSprite.workY = 370;
            return;
        }

        if (roomSlot == 12 || roomSlot == 14)
        {
            SpriteSpawnSecondary(SSPRITE_SAVE_YES_NO_CURSOR, 0, gCurrentSprite.spritesetGfxSlot, gCurrentSprite.primarySpriteRamSlot, 63, MESSAGE_BANNER_NO_X, 0);
        }
        ...
        if (roomSlot == 5 || roomSlot == 6 || roomSlot == 7 || roomSlot == 8 || roomSlot == 9)
            gCurrentSprite.workY = 180;
        else 
            gCurrentSprite.workY = 90;
    }
}
...
void SaveYesNoCursor(void)
{
    ...
    switch (gCurrentSprite.pose)
    {
        case 0:
            ...
            gCurrentSprite.hitboxLeft = -4;
            gCurrentSprite.hitboxRight = 4;
            gCurrentSprite.pOam = sMessageBannerYesNoCursorOam;
            gCurrentSprite.animationDurationCounter = 0;
            gCurrentSprite.currentAnimationFrame = 0;
            gCurrentSprite.pose = 2;
            gCurrentSprite.yPosition = 63;

            if (gSpriteData[primary].roomSlot == 12)
                gCurrentSprite.xPosition = MESSAGE_BANNER_YES_X;
            else
                gCurrentSprite.xPosition = MESSAGE_BANNER_NO_X;
            break;

        case 2:
            if (gChangedInput & KEY_LEFT)
            {
                if (gCurrentSprite.xPosition != MESSAGE_BANNER_YES_X)
                {
                    SoundPlay(0x1fa);
                    gCurrentSprite.xPosition = MESSAGE_BANNER_YES_X;
                }
            ...
            {
                if (gCurrentSprite.xPosition != MESSAGE_BANNER_NO_X)
                {
                    SoundPlay(0x1fa);
                    gCurrentSprite.xPosition = MESSAGE_BANNER_NO_X;
                }
