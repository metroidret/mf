...
void ProjectileUpdate(void)
{
    ...
    // Check spawn charging beam particle effect
    ...
    // Check spawn charging missile particle effect
    ...
    // Try to spawn the requested new projectile. First it checks for the limit for that kind of projectile,
    // then it tries to spawn it. If the spawn is successful, cooldown and other variables are set depending
    ...
    switch (gSamusData.newProjectile)
    {
        case PROJECTILE_CATEGORY_UNCHARGED_BEAM:
            ...
            {
                if (ProjectileCount(PROJ_TYPE_WAVE_BEAM, MULTI_PART_BEAM_LIMIT))
                {
            ...
            {
                if (ProjectileCount(PROJ_TYPE_PLASMA_BEAM, MULTI_PART_BEAM_LIMIT))
                {
            ...
            {
                if (ProjectileCount(PROJ_TYPE_WIDE_BEAM, MULTI_PART_BEAM_LIMIT))
                {
            ...
            {
                if (ProjectileCount(PROJ_TYPE_CHARGE_BEAM, MULTI_PART_BEAM_LIMIT))
                {
            ...
            {
                if (ProjectileCount(PROJ_TYPE_NORMAL_BEAM, BEAM_LIMIT))
                {
        ...
        case PROJECTILE_CATEGORY_MISSILE:
            ...
            {
                if (ProjectileCount(PROJ_TYPE_DIFFUSION_MISSILE, MISSILE_LIMIT))
                {
            ...
            {
                if (ProjectileCount(PROJ_TYPE_ICE_MISSILE, MISSILE_LIMIT))
                {
            ...
            {
                if (ProjectileCount(PROJ_TYPE_SUPER_MISSILE, MISSILE_LIMIT))
                {
            ...
            {
                if (ProjectileCount(PROJ_TYPE_NORMAL_MISSILE, MISSILE_LIMIT))
                {
        ...
        case PROJECTILE_CATEGORY_BOMB:
            if (ProjectileCount(PROJ_TYPE_BOMB, BOMB_LIMIT))
            {
        ...
        case PROJECTILE_CATEGORY_POWER_BOMB:
            if (ProjectileCount(PROJ_TYPE_POWER_BOMB, POWER_BOMB_LIMIT) && gCurrentPowerBomb.animationState == 0)
            {
        ...
        case PROJECTILE_CATEGORY_CHARGED_BEAM:
            ...
            if (beams & BF_WAVE_BEAM)
            {
                if (ProjectileCount(PROJ_TYPE_CHARGED_WAVE_BEAM, MULTI_PART_BEAM_LIMIT))
                {
            ...
            {
                if (ProjectileCount(PROJ_TYPE_CHARGED_PLASMA_BEAM, MULTI_PART_BEAM_LIMIT))
                {
            ...
            {
                if (ProjectileCount(PROJ_TYPE_CHARGED_WIDE_BEAM, MULTI_PART_BEAM_LIMIT))
                {
            ...
            {
                if (ProjectileCount(PROJ_TYPE_CHARGED_CHARGE_BEAM, MULTI_PART_BEAM_LIMIT))
                {
            ...
            {
                if (ProjectileCount(PROJ_TYPE_CHARGED_NORMAL_BEAM, BEAM_LIMIT))
                {
...
/**
 * @brief 819a0 | 4c | Draws all projectiles that don't have the 0x80 status flag set
 * 
 */
...
// FIXME: Rename function
/**
 * @brief 819ec | 4c | Draws all projectiles that have the 0x80 status flag set
 * 
 */
...
/**
 * @brief 82120 | cc | Moves the current projectile by the provided distance
 * 
...
void ProjectileMove(u8 distance)
{
    ...
    // Check add Samus's horizontal velocity if moving in the same direction
...
void ProjectileSetMissileTrail(u8 particle, u8 delay)
{
    ...
    {
        movement = 0x20;
        y = gCurrentProjectile.yPosition;
        x = gCurrentProjectile.xPosition;
...
void ProjectileCheckHittingSprite(void)
{
    ...
    if (gCurrentPowerBomb.animationState != 0 && gEquipment.weaponsStatus & MBF_POWER_BOMBS)
    {
        ...
        for (i = 0; i < MAX_AMOUNT_OF_SPRITES; i++)
        {
            if (gSpriteData[i].pose < SPRITE_POSE_TURNING_INTO_X)
            {
                ...
                if (gSpriteData[i].invincibilityStunFlashTimer & SPRITE_ISFT_POWER_BOMB_STUNNED)
                    continue;
    ...
    if (SpriteUtilCheckSamusDamagingPose())
        contactDamage = 0x1;
    else if (SpriteUtilCheckSamusPseudoScrew(0x80))
        contactDamage = 0x2;
...
void ProjectileBringSpriteToPowerBombCenter(u8 spriteSlot)
{
    ...
    if (!(gSpriteData[spriteSlot].invincibilityStunFlashTimer & SPRITE_ISFT_POWER_BOMB_STUNNED))
        return;
    ...
    else if (spriteY < powerBombY)
    {
        ...
        u32 tmp = QUARTER_BLOCK_SIZE + PIXEL_SIZE + 0x10000;
        tmp++, tmp--;
        spriteY += tmp;
    }
...
void ProjectilePowerBombHitSprite(u8 spriteSlot)
{
    ...
    if (SPRITE_GET_ISFT(gSpriteData[spriteSlot]) < isft)
        gSpriteData[spriteSlot].invincibilityStunFlashTimer = SPRITE_ISFT_POWER_BOMB_STUNNED | isft;
    else
        SPRITE_SET_ISFT(gSpriteData[spriteSlot], SPRITE_ISFT_POWER_BOMB_STUNNED);
}
...
u8 ProjectileIceMissileDealDamage(u8 spriteSlot, u8 projectileSlot, u16 damage)
{
    ...
    if (weakness & (WEAKNESS_SUPER_MISSILES | WEAKNESS_MISSILES))
    {
        ...
        if (SPRITE_GET_ISFT(gSpriteData[spriteSlot]) < flashTimer)
        {
...
void ProjectileDealDamageToZazabi(u16 damage)
{
    ...
    if (SPRITE_GET_ISFT(gCurrentSprite) < 17)
    {
...
void ProjectileChargedNormalBeamInit(void)
{
    ...
    gCurrentProjectile.hitboxRight = BLOCK_TO_SUB_PIXEL(0.25f);
...
void ProjectileNormalBeamInit(void)
{
    ...
    gCurrentProjectile.hitboxRight = BLOCK_TO_SUB_PIXEL(0.125f);
    gCurrentProjectile.status &= ~PROJ_STATUS_NOT_DRAWN;
...
void ProjectileMoveTumblingMissile(void)
{
    ...
    {
        offset = gCurrentProjectile.timer;
        movement = sTumblingMissileFallingVelocity[offset];
...
void ProjectileSuperMissileHandler(void)
{
    ...
    switch (gCurrentProjectile.movementStage)
    {
        case PROJECTILE_STAGE_INIT:
            ...
            gCurrentProjectile.hitboxRight = BLOCK_TO_SUB_PIXEL(0.1875f);
            ProjectileMissileInit(0);
...
void ProjectileDiffusionFlakeHandler(void)
{
    ...
    primarySlot = gCurrentProjectile.primaryProjectileSlot;
...
void ProjectileBombInit(void)
{
    ...
    gCurrentProjectile.hitboxRight = BLOCK_TO_SUB_PIXEL(0.5f);
    gCurrentProjectile.status &= ~PROJ_STATUS_NOT_DRAWN;
...
void ProjectilePowerBombInit(void)
{
    ...
    // Decrement power bombs, and if no power bombs, toggle off power bombs
...
void ProjectileChargedWideBeamInit(void)
{
    ...
    gCurrentProjectile.hitboxRight = BLOCK_TO_SUB_PIXEL(0.3125f);
    gCurrentProjectile.status &= ~PROJ_STATUS_NOT_DRAWN;
...
void ProjectileChargedWaveBeamInit(void)
{
    ...
    equippedBeams = gEquipment.beamStatus;
...
void ProjectileFlareInit(void)
{
    ...
    gCurrentProjectile.hitboxRight = BLOCK_TO_SUB_PIXEL(1.0f);
    gCurrentProjectile.status &= ~PROJ_STATUS_NOT_DRAWN;
    gCurrentProjectile.status |= PROJ_STATUS_ABOVE_BG1;
    gCurrentProjectile.movementStage++;
