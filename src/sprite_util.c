...
void SpriteUtilSamusAndSpriteCollision(void)
{
    ...
    statusCheckFlags = SPRITE_STATUS_EXISTS | SPRITE_STATUS_ONSCREEN | SPRITE_STATUS_HIDDEN;
    for (i = 0; i < MAX_AMOUNT_OF_SPRITES; i++)
    {
        status = gSpriteData[i].status;
        if ((status & statusCheckFlags) != (SPRITE_STATUS_EXISTS | SPRITE_STATUS_ONSCREEN))
            continue;
        ...
        else
        {
            switch (gSpriteData[i].samusCollision)
            {
                ...
                case SSC_KNOCKS_BACK_SAMUS:
                    if (gSamusData.invincibilityTimer == 0 && !SpriteUtilCheckSamusDamagingPose() && !SpriteUtilCheckSamusPseudoScrew(i))
                    {
                        if (collisionFlags & SPRITE_COLLISION_FLAG_ON_LEFT)
                ...
                case SSC_YAMEBA:
                    ...
                    if (gSamusData.invincibilityTimer < 8 && !SpriteUtilCheckSamusDamagingPose() && !SpriteUtilCheckSamusPseudoScrew(i))
                    {
                ...
                case SSC_E:
                    if (!SpriteUtilCheckSamusDamagingPose() && !SpriteUtilCheckSamusPseudoScrew(i))
                    {
                        if (collisionFlags & SPRITE_COLLISION_FLAG_ON_LEFT)
                        {
                            SpriteUtilCheckCollisionAtPosition(samusY, spriteLeft - gSamusData.drawDistanceRight + gSamusData.drawDistanceLeft);
 ...
 */
u32 SpriteUtilHasCurrentAnimationEnded(void)
{
    u8 adc;
    u16 caf;
 ...
 */
u32 SpriteUtilHasCurrentAnimationNearlyEnded(void)
{
    u8 adc;
    u16 caf;
 ...
 * @return u32 bool, ended
 */
u32 SpriteUtilHasAnimationEnded(u8 spriteSlot)
{
    u8 adc;
    u16 caf;
 ...
 * @return u32 bool, ended
 */
u32 SpriteUtilHasAnimationNearlyEnded(u8 spriteSlot)
{
    u8 adc;
    u16 caf;
 ...
 * @return u32 bool, ended
 */
u32 SpriteUtilHasSubSprite1AnimationEnded(void)
{
    u8 adc;
    u16 caf;
 ...
 * @return u32 bool, ended
 */
u32 SpriteUtilHasSubSprite1AnimationNearlyEnded(void)
{
    u8 adc;
    u16 caf;
 ...
 * @return u32 bool, ended
 */
u32 SpriteUtilHasSubSprite2AnimationEnded(void)
{
    u8 adc;
    u16 caf;
 ...
 * @return u32 bool, ended
 */
u32 SpriteUtilHasSubSprite2AnimationNearlyEnded(void)
{
    u8 adc;
    u16 caf;
...
void unk_121e0(u16 yPosition, u16 xPosition)
{
    ...
    if (gSpriteData[gCurrentSprite.primarySpriteRamSlot].status & SPRITE_STATUS_MOSAIC && gCurrentSprite.ignoreSamusCollisionTimer > 14)
...
/**
 * @brief 1271c | a0 | Checks if samus is doing a pseudo screw
 * 
 * @param spriteSlot Sprite slot
 * @return u32 bool, pseudo screw
 */
u32 SpriteUtilCheckSamusPseudoScrew(u8 spriteSlot)
{
    u8 pseudoScrew;
 ...
 * @return u8 Sprite ram slot
 */
u8 SpriteUtilFindSecondary(u8 spriteId)
{
    u8 i;
    u8 count;
 ...
 * @return u8 Count
 */
u8 SpriteUtilFindSecondaryWithRamSlot(u8 spriteId, u8 ramSlot)
{
    u8 i;
    u8 count;
